...
document.addEventListener('click', function (ev) {
    ...
    if (t.id === 'destroyBtn') {
        ...
        yfPost('<?= au('orders_destroy') ?>', fd).then(function (d) { alert(d.msg); if (d.code === 0) location.reload(); });
        return;
    }
    if (t.hasAttribute('data-del')) {
        if (!confirm(t.getAttribute('data-confirm') || '确定删除该订单?')) return;
        fd.append('id', t.getAttribute('data-del'));
        yfPost('<?= au('order_del') ?>', fd).then(function (d) { alert(d.msg); if (d.code === 0) location.reload(); });
        return;
    }
    if (t.hasAttribute('data-clean')) {
        if (!confirm(t.getAttribute('data-confirm') || '确定清理?')) return;
        fd.append('scope', t.getAttribute('data-clean'));
        yfPost('<?= au('order_clean') ?>', fd).then(function (d) { alert(d.msg); if (d.code === 0) location.reload(); });
        return;
    }
    fd.append('id', t.getAttribute('data-deliver'));
    yfPost('<?= au('order_deliver') ?>', fd).then(function (d) { alert(d.msg); if (d.code === 0) location.reload(); });
});
</script>
