...
<div class="lic-hero <?= $pro ? 'is-pro' : '' ?>">
    <div class="lic-hero-main">
        ...
        <div class="lic-sub">
            ...
            ?>
            <?php if ($license['license_key']): ?><span style="opacity:.65"> · 授权码 <?= e($license['license_key']) ?></span><?php endif; ?>
        </div>
    </div>
...
<div class="lic-grid">
    <div class="card">
        <h3>购买授权码</h3>
        <?php if ($pro): ?>
            <p class="dim" style="margin:0 0 12px">本站已激活专业版。可随时重新验证授权(需联网, 同站重复验证无副作用)。</p>
            <button class="btn gray" id="verify-btn">✓ 验证授权</button>
        <?php else: ?>
            <p class="dim" style="margin:0 0 12px">填写邮箱后跳转官方收银台, 支持 <b>支付宝 / 微信 / USDT(TRC20)</b>。支付成功后系统自动生成<b>永久授权码</b>: 本页直接展示并同步发送到您的邮箱。</p>
            <form class="form-inline" id="buy-form">
                <input type="email" id="buy-email" placeholder="接收授权码的邮箱" style="width:220px" required>
                <button class="btn" type="submit" style="height:42px;padding:0 26px;font-size:14px">⚡ 立即购买 ¥<?= e($price) ?></button>
            </form>
            <p class="dim" style="margin:10px 0 0">一个授权码可激活一个站点; 已有授权码可直接在右侧粘贴激活。</p>
        <?php endif; ?>
    </div>
    ...
    <div class="card">
        ...
        <p class="dim" style="margin:10px 0 0">向官方购买后获得授权码, 粘贴激活即可(支持离线校验, 无需联网)。注意: 一码仅绑定一个站点, 激活后不可更换。</p>
    </div>
</div>
