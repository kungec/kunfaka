...
<div class="panel">
    ...
    <form class="buy-form" method="get" action="<?= u('order/query') ?>">
        ...
        <div class="form-row">
            <button class="btn-buy" type="submit" style="min-width:140px">查询</button>
        </div>
    </form>
        ...
        <div class="order-list">
            <?php foreach ($orders as $o): ?>
                <a class="order-item" href="<?= u('order/detail', ['sn' => $o['sn']]) ?>">
                    ...
                    <div class="oi-line dim mono"><?= e($o['sn']) ?> · ¥<?= e(nf($o['total'])) ?> · <?= e(date('m-d H:i', $o['created_at'])) ?></div>
                </a>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
    ...
    <?php if ($mode === 'contact' && $orders && !$verified): ?>
            ...
            <div class="buy-form" style="margin-top:14px">
                ...
                <?php if ($otpStep): ?>
                    <form class="buy-form" method="post" action="<?= u('order/query') ?>">
                        ...
                        <div class="form-row">
                            <button class="btn-buy" type="submit" style="min-width:140px">验证并查看订单</button>
                        </div>
                    </form>
                    <form method="post" action="<?= u('order/query') ?>">
                        ...
                        <button class="btn-buy" type="submit" style="min-width:140px;background:transparent;color:inherit;border:1px solid currentColor">重新发送验证码</button>
                    </form>
                <?php else: ?>
                    <form method="post" action="<?= u('order/query') ?>">
                        <?= csrf_field() ?>
                        ...
                        <input type="hidden" name="oq_action" value="send">
                        <div class="form-row">
                            <button class="btn-buy" type="submit" style="min-width:140px">发送验证码到该邮箱</button>
                        </div>
                    </form>
                <?php endif; ?>
            </div>
        ...
        <?php endif; ?>
    <?php elseif ($mode === 'contact' && !$orders && $kw !== ''): ?>
        <div class="empty-box"><p>没有找到相关订单 (｡•́︿•̀｡)</p></div>
    <?php elseif ($mode === 'sn' && !$orders && $kw !== ''): ?>
        <div class="empty-box"><p>没有找到相关订单 (｡•́︿•̀｡)</p></div>
    <?php endif; ?>
</div>
